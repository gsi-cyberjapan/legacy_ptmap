...
webtis.SERVER_URL = {
		...
		OVERLAY_MAP_SERVER : 'http://cyberjapandata3.gsi.go.jp/mapproxy/gsi/',
        OVERLAY_MAP_TRANSPARENT_IMAGE : 'http://portal.cyberjapan.jp/test_v4a/sys/v4a/image/transparent.png'
};
...
webtis.Control.MultiLayerDragFeature = OpenLayers.Class(OpenLayers.Control.DragFeature, {
	...
	deactivate: function () {
		if (this.active) {
			if(this.layers) {
            	if (this.map.getLayerIndex(this.layer) != -1) {
...
//	url2 : webtis.SERVER_URL.BASEMAP_TILE_SERVER2+"/${did}/latest/${z}${dir}/${x}${y}.${ext}",					//特定のファイル群だけはこっちのサーバ
	...
	getURL : function(bounds) {
		...
		if (currentData.endUse) {
			...
			var revision = "latest";
			var url = curMetaData.serverURL+"/${did}/"+revision+"/${z}${dir}/${x}${y}.${ext}";
			
			if((xyz.did.match(/^[0-4][0-9]SABO$/))||(xyz.did.match(/^[0-4][0-9][0-9][0-9][0-9]DM$/)))	{		//DM（都市計画基図、砂防基盤図）の判定
				url = this.urlDM;
			}else if(xyz.did=='LCM25K')	{
				url = this.url3;
	...
	updateAttribution : function() {
		if (this.map) {
			...
			if ((dataId=='BAFD1000K2')||(dataId=='BAFD200K2')||(dataId=='BAFD1000KG')||(dataId=='BAFD200KG')||(dataId=='')) {
	 ...
	 * 
     * Returns:
     * <Object> 現在設定されているデータセットの情報が格納されたObject
	 */
	getDataSet : function() {
		return this.dataSet;
	},
	...
	getAllMetaData : function() {
		...
		for (var z = this.minZoomLevel; z <= this.maxZoomLevel; z++) {
			var curDataSet = this.dataSet[z];
			var curMetaData = undefined;
			if (curDataSet) {
				if (this.metaData) {
					curMetaData = this.metaData[curDataSet.dataId];
				}
			}
			if (curMetaData) {
				byScales[z] = curMetaData;
			}
		}
		return byScales;
	},
...
OpenLayers.Util.onImageLoadError = function(){
	...
	var overlayMapIds = ['lcm25k', 'meijiswale', 'akandake', 'tokachidake', 'tarumaesan', 'usuzan', 'komagatake', 'kurikomayama', 'adatarayama', 'bandaisan', 'izuoshima', 'miyakezima', 'kusatsushiranesan', 'fujisan', 'ontakesan', 'kujirenzan', 'asosan', 'unzendake', 'kirishimayama', 'sakurazima', 'satsumaiojima',
						 ...
						 'kinki2008','kinki2001','kinki1996','kinki1991','kinki1985','kinki1979','kinki1974'];
	...
	for (var i in overlayMapIds){
		...
		if((dataId=='LCM25K')||(dataId=='SWALE')||(dataId=='16akandake')||(dataId=='02tokachidake')||(dataId=='10tarumaesan')||(dataId=='09usuzan')||(dataId=='05komagatake')||(dataId=='00kurikomayama')||(dataId=='14adatarayama')||(dataId=='11bandaisan')||(dataId=='13izuoshima')||(dataId=='06miyakezima')||(dataId=='03kusatsushiranesan')||(dataId=='12fujisan')||(dataId=='00ontakesan')||(dataId=='15kujirenzan')||(dataId=='04asosan')||(dataId=='07unzendake')||(dataId=='08kirishimayama')||(dataId=='01sakurazima')||(dataId=='17satsumaiojima')||(dataId.match(/(capital|chubu|kinki)[0-9][0-9][0-9][0-9]/))){
